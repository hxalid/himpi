...
if __name__ == '__main__':
    ...
    for g in range(1, p):
        comm_msgs = {}
        if (p%g==0):
            for alg_in in range(0, 7):
                for alg_out in range(0, 7):
                    file_name = './silgetsin/ompi_conf_g%s_alg_in%s_alg_out%s' % (g, alg_in, alg_out)
                    
                    comm_sizes = []
                    if (g>p/g):
                        comm_sizes.append(int(p/g))
                        comm_sizes.append(g)
                    else:
                        comm_sizes.append(g)
                        comm_sizes.append(int(p/g))
...
# [message_size alg topo segmentation] 
    ...
    HMPI_Runner.create_config_file('fayildi-bu', ['7'], ['0', '16'], {'0':msg_sizes_p0, '16':msg_sizes_p16})
