...
const int C_HSCATTER = 1;
const int C_HGATHER = 2;
...
//    int my_group = 0;
	...
	int hcol = C_HSCATTER;
	int opt;
	while ((opt = getopt(argc, argv, "m:r:s:i:a:c:g:f:d:wuh")) != -1) {
		switch (opt) {
		case 'm':
		...
		case 'h':
			if (rank == 0)
				fprintf(stdout, "%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n",
						...
						"-s scatter/gather root",
						"-i number of iterations",
						"-c hcoll id, Desfault is 1 (hscatter). 2 is hgather"
						"-g maximum number of groups",
						"-d set to 0 disable host names. Default is 1",
						"-w if used don't create sub-communicators", 
						"-h help");
			MPI_Finalize();
			return 0;
		default:
	...
	if (comm_measure != MPI_COMM_NULL) {
		...
		if (rank == root) {
			fprintf(stdout,
					"Inputs: hcol=%d, p=%d, p_new=%d, root=%d, m=%d, f=%d, rec=%d, reps=%d, d=%d, dont_use_comm_world=%d, rec_comm_world=%d\n",
					hcol, num_proc, num_proc_new, root, msg_size, msg_factor, rec,
					reps, debug, dont_use_comm_world, rec_comm_world);

			fprintf(stdout,
					"hcol  root  p   g    msg(KB)     time     rec   reps   rec_in  rec_out  rec_world\n");
		}
		...
		if (rec != 0) {
			...
			while (groups < g_max) {
				if (num_proc_new % groups == 0) {
					...
					} else if (hcol == C_HGATHER) {
					   if (rank == root) {
					      rcv_array = create_rand_elms(msg_size);
					   }
					   snd_array = (int*) malloc(sizeof(int) * msg_size / num_proc_new);
					   assert(snd_array != NULL);
					} else {
					   fprintf(stdout, "Wrong collective id\n");
					   MPI_Abort(comm_measure, -1);
					}
					...
					for (i = 0; i < reps; i++) {
						...
						if (hcol == C_HSCATTER) {
						   hcol_res = MPI_HScatter(snd_array, msg_size / num_proc_new,
								MPI_INT, rcv_array, msg_size / num_proc_new, MPI_INT, root,
								comm_measure, groups);
						} else if (hcol == C_HGATHER) {
						    hcol_res = MPI_HGather(snd_array, msg_size / num_proc_new, MPI_INT, rcv_array, msg_size / num_proc_new,
								    MPI_INT, root, comm_measure, groups);
						} 
					...
					if (rank == root) {
						fprintf(stdout, "%d %d %d %d %f %f %d %d %d %d %d\n", 
								hcol,
								root,
								num_proc_new, groups,
								msg_size * sizeof(int) / 1024., max_time, rec,
