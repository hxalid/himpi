...
double lu_factorize(LU_data* lu_data, Debug_data* debug_data) {
	...
	{ // Validation block
		...
		if (prow * pcol != nproc) {
			if (!me)
				fprintf(stderr,
						"num_rows * num_cols should be equal to nproc.\n");
			MPI_Finalize();
			exit(0);
		}
	...
	if (print_proc_view)
		if (me == 0) {
			for (i = 0; i < n; i++) {
				for (j = 0; j < n; j++) {
					diagonal_grid = bc_block_owner(i, j, block_size, block_size,
							prow, pcol);
					fprintf(stdout, "P%d (%d, %d) ", diagonal_grid.pid,
							diagonal_grid.pi, diagonal_grid.pj);
				}
				fprintf(stdout, "\n");
			}
		}
	...
	MPI_Aint disps[3] = { offsetof(dbl_twoindex, val), offsetof(dbl_twoindex,
			rank), offsetof(dbl_twoindex, posn) };
	int lens[3] = { 1, 1, 1 };
	...
	for (i = 0; i < nblocks; i++) {
		...
		for (ib = 0; ib < block_size; ib++) {
			...
			diagonal_grid = bc_block_owner(i * block_size + ib,
					i * block_size + ib, block_size, block_size, prow, pcol);

			if (col == pj) {
				if (with_partial_pivoting) {
					if (unprocessed_row < n_loc) {
						r = max_col_loc(a, n_loc,
								unprocessed_row
										+ ib * (me == diagonal_grid.pid),
								unprocessed_col + ib);
						if (r == -1) {
							fprintf(stderr,
									"Something is wrong, me=%d, i=%d, ib=%d, unprocessed_col=%d, unprocessed_row=%d\n",
									me, i, ib, unprocessed_col,
									unprocessed_row);
							exit(-1);
						}
					...
					MPI_Allreduce(&local, &global, 1, mpi_dbl_twoindex,
							mpi_maxloc_dbl_twoindex, col_comm);
					...
					if (me == max_owner
							&& y.val
									== a[y.pvtline * n_loc + unprocessed_col
											+ ib])
						max_sign = 1;

					MPI_Bcast(&max_sign, 1, MPI_INT, max_owner / prow,
							col_comm);
					y.val *= max_sign;
					...
					y.pvtline = unprocessed_row
							+ ib * (me == diagonal_grid.pid);
					y.val = a[y.pvtline * n_loc + unprocessed_col + ib];
					MPI_Bcast(&y, 1, MPI_DOUBLE_INT, diagonal_grid.pi,
							col_comm);
					fprintf(stdout,
							"Running without pivoting: [i:%d, ib:%d, me:%d, y.val:%f]\n",
							i, ib, me, y.val);
				}
			} // End of (col == pj)
			...
			if (me == diagonal_grid.pi && with_partial_pivoting) {
				...
				int global_pivot_row = get_global_row(y.pvtline / block_size,
						y.pvtline % block_size, max_owner / prow, prow,
						block_size);
				if (step_print_pvt)
					printf(
							"i=%d, ib=%d, global_pivot_row=%d, y.val=%f, me=%d\n",
			...
			if (step_pivoting) { // Block of broadcasting and exchanging pivot
				...
				if (with_partial_pivoting) {
					MPI_Bcast(&y, 1, MPI_DOUBLE_INT, diagonal_grid.pi,
							row_comm);
					MPI_Bcast(&max_owner, 1, MPI_INT, diagonal_grid.pi,
							row_comm);

					if (row == max_owner / prow && row == diagonal_grid.pi
							&& y.pvtline == diagonal_grid.loc_i) {
						// do nothing

					} else if (row == max_owner / prow
							&& row == diagonal_grid.pi) {
						// local exchange
						swap_with_pivot_row(diagonal_grid.loc_i, y.pvtline,
								n_loc, a);
						...
						MPI_Send(&a[y.pvtline * n_loc], n_loc, MPI_DOUBLE,
								diagonal_grid.pi, PIVOT_TAG, col_comm);
						MPI_Recv(&a[y.pvtline * n_loc], n_loc, MPI_DOUBLE,
								diagonal_grid.pi, NON_PIVOT_TAG, col_comm,
								&status);
						 ...
						 */
						double* current_row_buffer = (double *) malloc(
								n_loc * sizeof(double));
						MPI_Status status;

						MPI_Recv(current_row_buffer, n_loc, MPI_DOUBLE,
								max_owner / prow, PIVOT_TAG, col_comm, &status);
						MPI_Send(&a[diagonal_grid.loc_i * n_loc], n_loc,
								MPI_DOUBLE, max_owner / prow, NON_PIVOT_TAG,
								col_comm);

						int il;
						for (il = 0; il < n_loc; il++)
							a[diagonal_grid.loc_i * n_loc + il] =
									current_row_buffer[il];
				...
				if (step_dvide) {
					if (col == pj
							&& (unprocessed_row < n_loc
									&& unprocessed_col < n_loc)) {
						...
						row_to_dvide = (unprocessed_row
								+ (me == diagonal_grid.pid))
								+ ib * (me == diagonal_grid.pid);

						for (ipr = row_to_dvide; ipr < n_loc; ipr++) {
							factor = a[ipr * n_loc + (unprocessed_col + ib)]
									/ (double) y.val;
							a[ipr * n_loc + (unprocessed_col + ib)] = factor;
							col_to_factor = unprocessed_col
									+ (col == diagonal_grid.pj) + ib;
							...
							for (ifc = col_to_factor;
									ifc < unprocessed_col + block_size; ifc++)
								a[ipr * n_loc + ifc] -= factor
										* a[unprocessed_row + ifc];
		...
		if (step_l0_u1) { // Start block of calculation of L0 and U1
			...
			if (row == pi) {
				 ...
				 */
				int max_num_nnz = (block_size) * (block_size + 1) / 2
						- block_size;
				double *l0_partial_buffer = (double*) malloc(
						max_num_nnz * sizeof(double));

				int l0_i, l0_j, l0_k = 0;
				if (col == pj) {
					for (l0_i = unprocessed_row;
							l0_i < unprocessed_row + block_size; l0_i++) {
						for (l0_j = unprocessed_col;
								l0_j < unprocessed_col + block_size; l0_j++) {
							if (l0_i > l0_j) {
								l0_partial_buffer[l0_k++] = a[l0_i * n_loc
										+ l0_j];
							}
						}
					}
				...
				int u1_start_col =
						(me == diagonal_grid.pid) ?
								unprocessed_col + block_size : unprocessed_col;
				double* u1_buffer = (double*) malloc(
						block_size * (n_loc - u1_start_col) * sizeof(double));

				//TODO. should I check col==pj ??
				int u1_i, u1_j, u1_k = 0;
				for (u1_i = unprocessed_row;
						(u1_i < unprocessed_row + block_size); u1_i++) {
				 ...
				 * Bcast L0 right
				 */
				MPI_Bcast(l0_partial_buffer, max_num_nnz, MPI_DOUBLE,
						diagonal_grid.pi, row_comm);

				//TODO:  end benchmarking bcast

				double *l0_buffer = (double*) malloc(
				...
				for (l0_i = 0; l0_i < block_size; l0_i++) {
					for (l0_j = 0; l0_j < block_size; l0_j++) {
						if (l0_i == l0_j)
							...
							l0_buffer[l0_i * block_size + l0_j] =
									l0_partial_buffer[l0_k];
							l0_k++;
						}
					}
				}
				...
				if (ldb != 0) {
					...
					// L0xU1 = C
					cblas_dtrsm(order, side, uplo, trans, diag, lda, ldb, alpha,
							l0_buffer, lda, u1_buffer, ldb);
		...
		if (step_bcast_l_u) {
			int u_start_col =
					(col == diagonal_grid.pj) ?
							unprocessed_col + block_size : unprocessed_col;
			double* u_buffer = (double*) malloc(
					block_size * (n_loc - u_start_col) * sizeof(double));
			...
			if (u_start_col < n_loc) { //If not, it means there is no one to bcast ??
				int u_i, u_j, u_k = 0;
				for (u_i = unprocessed_row;
						(u_i < unprocessed_row + block_size); u_i++) {
				...
				//TODO:  start benchmarking bcast
				MPI_Bcast(u_buffer, block_size * (n_loc - u_start_col),
						MPI_DOUBLE, diagonal_grid.pi, col_comm);
				//TODO:  end benchmarking bcast
			}

			int l_start_row =
					(row == diagonal_grid.pi) ?
							unprocessed_row + block_size : unprocessed_row;
			double* l_buffer = (double*) malloc(
					(n_loc - l_start_row) * block_size * sizeof(double));
			...
			if (l_start_row < n_loc) {
				...
				for (l_i = l_start_row; l_i < n_loc; l_i++) {
					for (l_j = unprocessed_col;
							(l_j < unprocessed_col + block_size); l_j++) {
				...
				//TODO:  start benchmarking bcast
				MPI_Bcast(l_buffer, (n_loc - l_start_row) * block_size,
						MPI_DOUBLE, diagonal_grid.pj, row_comm);
				//TODO:  end benchmarking bcast
			}
			...
			if (step_dgemm) {
				if (l_start_row < n_loc && u_start_col < n_loc) {
					...
					cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
							// m, n, k
							n_loc - l_start_row, n_loc - u_start_col,
							block_size, alpha,
							// A
							l_buffer,
							// lda
...
void print_matrix(int is, int ib, int rank, int n, int m, double* a,
		char* matrix_name) {
	int i, j;
...
void init_matrix(double** p_a, int n_loc, int m_loc, int row, int col, int rank,
		int do_ugly) {
	int x, z;
	int lda = m_loc;
	double *a;
