...
void *signal_arrival_of_data_loop(void *args) {
	...
	*/
	...
	if (t->rank == 0) {
		for (j=0; j<CHUNKS;j++) {
			for (i=0; i<NUM_CHANNELS; i++) {
				if (interested_peers[i] != t->rank) {
					MPI_Isend(&j, 1, MPI_INT, interested_peers[i], 2, t->comm, &request[i]);
				}
				else
...
void *get_data(void *args) {
	...
	while (1) {
		...
		if (interesting_bit != 0) {
			...
			MPI_Sendrecv(&alien_index, 1, MPI_INT, status.MPI_SOURCE, 3, &local_buffer[alien_index *  CHUNK_SIZE],  CHUNK_SIZE , MPI_CHAR, status.MPI_SOURCE, 4, t->comm, MPI_STATUS_IGNORE);
