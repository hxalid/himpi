...
-M			this flag is required for multi-core experiments\n");
	...
	MPIB_shared_initialize initialize = dlsym(handle, MPIB_SHARED_INITIALIZE);
	if (initialize != NULL) {
	...
	if (strstr(operation, "Alltoall"))
		container = (MPIB_coll_container*)MPIB_Alltoall_container_alloc((MPIB_Alltoall)func);
	...
	MPIB_shared_finalize finalize = dlsym(handle, MPIB_SHARED_FINALIZE);
	if (finalize != NULL) {
