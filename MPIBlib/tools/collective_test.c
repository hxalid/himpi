...
-O S			collective operation defined in the shared library (required: the name must contain Bcast, Scatter, etc)\n"
	...
	void (*func) = dlsym(handle, MPIB_SHARED_INITIALIZE);
	if (func != NULL)
		((MPIB_shared_initialize)func)(MPI_COMM_WORLD, subopts);
	if (rank == 0)
	...
	func = dlsym(handle, MPIB_SHARED_FINALIZE);
	if (func != NULL)
		((MPIB_shared_finalize)func)(MPI_COMM_WORLD);
	dlclose(handle);
